      ...
      >
        ...
        >
          <div class="flex items-center justify-between mb-4">
            ...
            >
              Approved by: {{ participant.moderator_accepted.name }}
            </a>
          </div>
          <div class="flow-root">
            <ul
...
export default {
  ...
  watch: {
    search: function () {
      const _this = this;
      this.isLoading = true;
      this.search = this.search.replace(/^(05)/g, '9665');
      axios
        .post('/api/admin/moderation/approved?search=' + this.search)
        .then((result) => {
  ...
  methods: {
    fetchParticipants() {
      ...
      if (this.search !== '') {
        axios
          .post(
            '/api/admin/moderation/approved?page=' +
